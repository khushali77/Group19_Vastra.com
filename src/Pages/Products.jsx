...
function DisplayCard(props){
    ...
    const addtofav = async (itemid,e) => {
        ...
        e.preventDefault()
        try {
            await axios.post(`http://localhost:4000/api/userfav/addtofav/${userid}/${itemid}`) 
            .then(res=>console.log(res.data));
        ...
        <hr size="10"></hr>
            <Link to='/product' onClick={(e)=>{getData(props.data._id,e)}}><Card.Body>
                ...
                <div className="product__details__btn">
                    ...
                    <button type = "button" className = "butn" onClick={(e)=>{addtofav(props.data._id,e)}}><img src = {check} className = "heart"  alt = "checked"/></button>
                </div>
                </div>
            </Card.Body></Link>
        </Card>
